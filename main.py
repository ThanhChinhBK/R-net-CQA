...
def train_SemEval(config):
    ...
    parser = get_record_parser_SemEval(config)
...
def evaluate_batch_SemEval(model, num_batches, sess, data_type, handle, str_handle, cb):
    ...
    map_sum = tf.Summary(value=[tf.Summary.Value(
        tag="{}/map".format(data_type), simple_value=metrics["map"]), ])
    mrr_sum = tf.Summary(value=[tf.Summary.Value(
        tag="{}/mrr".format(data_type), simple_value=metrics["mrr"]), ])
    return metrics, [loss_sum, map_sum, mrr_sum]
...
def evaluate_batch(model, num_batches, eval_file, sess, data_type, handle, str_handle):
    ...
    for _ in tqdm(range(1, num_batches + 1)):
        qa_id, loss, y = sess.run(
            [model.qa_id, model.loss, model.y], feed_dict={handle: str_handle})
